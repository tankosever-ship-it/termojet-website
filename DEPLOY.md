...
> ⚠️ **Ре-імпорт товарів (з 1С/WP) стирає ці зміни.** Пере-імпорт перезаписує базовий `seo_title`
> (UA) → дедуп (200)/(240) і НГ-52.150 Л зникають, повертаються дублі-title. FAQ і EN-скорочення
> (в `i18n`) зазвичай виживають, а UA-`seo_title` — ні. **Після кожного ре-імпорту** ганяй
> `seo-fix-titles.cjs --apply` (UA) і `fix-seo-titles-2026-09.cjs` (усі 6 мов) — обидва ідемпотентні.
>
> ⚠️ **04.09.2026 той самий баг знайшовся в сусідній `migrateCurrency()`** — вона безумовно
> перезаписувала `products.currency` зі seed щостарту. Оскільки `price` не чіпає жодна міграція,
> пара розʼїжджалась: товар із гривневою ціною 73 000 отримував `currency='EUR'` і сторінка
> показувала 3.87 млн грн. Теж переведено на backfill лише порожніх. Аудит решти міграцій:
> `seedProducts`/`seedBlog`/`seedPortfolio` сіють лише в порожню таблицю, `migrateI18n` і
> `migratePortfolioCols` лише додають колонки, `migrateSpecsArticle` бере `sku` з самої БД, не зі
> seed. **Правило: будь-яка міграція, що читає `seed-*.json`, має ЗАПОВНЮВАТИ порожнє, а не
> перезаписувати живе** — інакше вона тихо їсть правки з адмінки.

> ✅ **Виправлено 03.09.2026: рестарт більше НЕ стирає ці поля.** Раніше винен був не лише
> ре-імпорт: `migrateSeo()` у `backend/db/index.js` виконується щостарту й робила БЕЗУМОВНИЙ
> `UPDATE seo_title/meta_description` зі `seed-products.json`. Тобто кожен
> `docker compose up --build` мовчки відкочував ці поля — разом із правками, зробленими
