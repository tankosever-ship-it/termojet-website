...
## Сесія 2026-09-03 — Тимчасові гривневі ціни на два насоси APM-F (2 тижні)

> Запит власника: поставити на дві позиції фіксовані ціни в гривні на два тижні, потім автоматично повернути попередні.
...
- **Ціни за курсом на 04.09.2026** (НБУ 51.9382 × 1.022 = 53.0808): `65/12-340` показував 71 128 грн → стає **73 000** (+2.6 %); `40/12-250` показував 62 635 грн → стає **63 830** (+1.9 %). Це підвищення, не знижка — тому `price`, а не `sale_price` (інакше сайт намалював би «акцію» з закресленою нижчою ціною).
...
- **Автоповернення 17.09.2026** — щоденний cron `root` о 06:00 запускає `--auto`; той мовчить, поки строк не вийшов. Щоденний, а не одноразовий: пропущений запуск (сервер лежав, контейнер перезбирався) наздоганяється наступного дня.
...
- **Перевірено локально на копії прод-БД** (`VACUUM INTO`): apply → status → auto до строку (мовчить) → auto після строку з ручною правкою одного товару (другий повернувся, перший пропущений, стан збережено) → `--revert --force` (обидва повернулись у 1340/1180 EUR) → повторний `--auto` без стану (мовчить).

### Google Merchant: мовні фіди вели на україномовні URL
...
- Додано `LANG_PREFIX` (дзеркало `LANG_PREFIX` у `backend/server.js`); `g:link` і `<link>` каналу тепер ведуть на `/en|/pl|/de|/fr|/ro`. Українському фіду префікс порожній — його URL не змінились.
- Перед правкою перевірено, що цілі існують: усі 6 мовних URL товару віддають **200**, **самоканонічні**, `robots: index, follow`, hreflang×7. Тобто фід тепер веде на індексовані сторінки потрібною мовою, а не на редирект.
- До сьогоднішньої зміни цін стосунку не має — баг давній.

### 🔴 migrateCurrency затирала валюту щостарту — ціна на вітрині стала 3.87 млн грн
- **Знайдено через 4 хв після деплою фікса фідів** — статус кампанії показав «73 000 **EUR**» замість «73 000 грн».
- **Причина — той самий клас бага, що вчора в `migrateSeo()`, у сусідній функції:** `migrateCurrency()` робила БЕЗУМОВНИЙ `UPDATE products SET currency = ?` зі `seed-products.json` щостарту (коментар прямо казав «seed — джерело істини, самовиправляється щостарту»). Вчора виправлено `migrateSeo`, а цю пропущено.
- **Чому вийшло так дорого:** ціну не чіпає жодна міграція, а валюту чіпала. Пара розʼїхалась — `price=73000` з `currency='EUR'` → сторінка й JSON-LD показали **3 874 901 грн**. Тобто баг не просто відкочував зміну, а робив гірше, ніж було до неї.
...
- **Друга лінія захисту в `temp-price.cjs`:** щоденний `--auto` тепер ще й звіряє цілісність. Якщо ціна кампанійна, а валюта інша — це підпис машинної псувальниці (ніхто не міняє в адмінці саму лише валюту, зберігши акційну суму), тож валюта повертається з гучним записом у лог. Якщо ж змінилась і ціна — це схоже на свідому правку менеджера, і сторож мовчить. Обидві гілки перевірені на копії прод-БД.
