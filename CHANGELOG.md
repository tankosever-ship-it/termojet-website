...
## Сесія 2026-09-11 (iii) — Категорія «Пелетні котли» + Ignis Pro 15 Plus

> Рішення власника: заводимо лише виконання Plus, ціна **139 000 грн**, категорія
> передостання — перед «Акція». Дані з інструкції (64 стор.) і брошури A4, зібраних
> у `~/CODE/pipelines/manuals/ignis-pro-15/`; числа там зведені з протоколів ZETOM.

### 🗂 Категорія (вісім місць, не сім)

- `categories.js` (6 мов), `CATEGORY_META` у `server.js`, `CAT` + `GPC` у `merchant.js`,
  `CATEGORY_BANNERS` у `CatalogPage.jsx`, банер у `public/`, `DOCS_BY_CATEGORY`.
- **Плюс восьме, якого не було в ТЗ:** `CategoryIcon.jsx` тримає **явну мапу** іконок
  (namespace-імпорт lucide вимикав tree-shaking і тягнув 607 кБ). Без запису `Flame`
  іконка категорії мовчки не малювалась би.
- `CATEGORY_META` — не лише SEO: `isKnownRoute()` перевіряє саме цей об'єкт, і без
  запису `/catalog/peletni-kotly` віддавав би **HTTP 404** при живій сторінці.
- `google_product_category` = **3082** (`Home & Garden > … > Furnaces & Boilers`).
  У гілці `Hardware > HVAC`, куди вказувало ТЗ, котлів немає взагалі — перевірено
  за офіційним `taxonomy-with-ids`.

### 🔥 Товар `IGNIS-PRO-15-PLUS`

- `apply-peletni-kotly.js` + `peletni-kotly-data.json`: 139 000 ₴ (UAH — на сторінці
  стоїть рівно ця цифра; зворотний бік у тому, що котел імпортний і при зміні євро
  ціну доведеться правити руками), підкатегорія «15 кВт», 21 характеристика,
  опис із брошури, переклади 6 мовами зі штампом `_srcHash`.
- Фото: рендери з інструкції на квадратному полотні 1200×1200 (+webp) — головне
  фото, розріз, панель керування, комплект поставки. Тимчасові, до студійних.
- Документи: інструкція (id 197, «Інструкції») і брошура (id 198, **«Брошури»**,
  а не «Каталоги та прайси» — картка товару показує лише перші дві категорії).

### 💰 Фід так, прайс ні

- У `generate.py` з'явився **другий** список винятків. `FEED_EXCLUDE` (труба SPY)
  відсіює товар одразу після завантаження каталогу — він зникає звідусіль.
  `PRICE_EXCLUDE` (котел) застосовується лише на записі XLSX і CSV, тому у фіді
  маркетплейсів позиція лишається й зі сторінкою не розходиться.
- Додано розділ «Пелетні котли» в `SECTIONS`/`section_for`/`SECTION_IMAGE` — інакше
  `product_type` у фіді поїхав би як «Додаткове обладнання».

### 🖼 Розріз котла в галереї — був із порожніми виносками

`figures.py` у конвеєрі інструкції вибілює нутро рамок, а підписи друкує поверх
уже в `src/fig/*.html`. Я взяв у галерею растр із `src/assets/` — і на сайті
рисунок стояв із **порожніми квадратиками**, тоді як в інструкції той самий
рисунок правильний. Перезняв картинку з `src/fig/budova-rozriz.html` (headless
Chrome + шрифт інструкції) — усі дев'ять підписів на місці.

### 🔗 Перелінковка котла з рештою каталогу

- В описі котла з'явився розділ **«Що потрібно для обв'язки»** — вісім посилань,
  однакові в усіх шести мовах (у не-українських із мовним префіксом). Добір не
  «схожі товари», а вузли зі схем обв'язки самої інструкції, і кожна пара
  перевірена числом: 15 кВт при перепаді 15 K це ≈0,86 м³/год, звідси насос
  APE 25/60-180; ГС-25 тримає 25 кВт при Δt10; колектор Mini 2+1 — теж 25 кВт;
  TMV132 (35–60 °C) — єдина наша лінійка, що накриває вимогу «зворот не нижче 55 °C».
- **Зворотні посилання** (`apply-perelinkuvannya-kotel.js`): по одному реченню
  в п'ять карток-компаньйонів, шістьма мовами, з переставленим `_srcHash`.
  Навіщо: у котла лише два входи — меню й sitemap, а блок «Схожі товари» бере
  товари **лише з тієї самої категорії**, тож поки котел у ній один, блок порожній.
- Чого свідомо не лінкуємо: запобіжний клапан 1,5 бар, термоманометр,
  розширювальний бак і сітчастий фільтр. Вони є на схемах, але в каталозі їх
  немає — у картці лишається чесне «добирає монтажник», без посилання в нікуди.

### 🏷 KHT прибрано з документів і з картки

- Рішення власника того ж дня: марки колишнього виробника не має бути ні в
  інструкції, ні в брошурі. Прибрано п'ять місць у `~/CODE/pipelines/manuals/ignis-pro-15/`
  (знак виробника на сторінці «Відомості про виріб», його реквізити в тій самій
  таблиці, згадка в умовах гарантії, колофон, підвал брошури), PDF перезібрано.
  У «Відомостях про виріб» виробником тепер стоїть **Termojet** — як у решті партії.
- `BRAND_WORDS` у конвеєрі інструкції тепер ловить і саму марку, тож забута згадка
  впаде на перевірці, а не поїде у друк. Числа `588` і `015` з адреси виробника
  занесено в `IGNORE_NUMBERS` — інакше звірка чисел щоразу звітувала б про втрату.
- На картці товару атрибуцію теж прибрано в усіх шести мовах: сторінка не може
  писати «виробник KHT GLOBAL», роздаючи з себе ж інструкцію, де стоїть Termojet.

### 🩹 Побічно полагоджено: обрубаний title категорії

`slice(0, 60)` різав титул по символах, і румунська назва давала
`…pentru centrale termice | Ter` — у видачі хвіст без бренду. Тепер ріжеться
**обгортка і по слову** (`padCategoryTitle`); решта категорій не змінилась.

## Сесія 2026-09-11 (ii) — Вітринна труба SPY PE-RT 16×2

> Прохання партнерів: показати у себе в країні, що ця сама труба продається і в Україні.
> Джерело — cazan.md (7 MDL/м ≈ 18 грн), у нас 20 грн/м. Картка вітринна: у каталозі є,
> у рекламі й у прайсі для партнерів — немає.

### 🧾 Товар

- `backend/scripts/apply-truba-spy.js` + `truba-spy-data.json` — артикул `SPY-16X2`,
  категорія «Система підлогового опалення», 20 грн/м, бухта 600 м, кисневий бар'єр, ЄС.
  Пише і в `seed-products.json`, і в живу БД; `--remove --apply` прибирає картку назад.
- Переклади **шістьма мовами** вкладені в дані й кладуться в колонку `i18n` разом зі
  штампом `_srcHash` — інакше наступний прогін `translate-content.js` вважав би рядок
  неперекладеним і перетер ручний переклад машинним.
- Румунська назва — **точно як у партнера**: `Teava podea calda cu bariera de Oxigen PE-RT 16X2 SPY`.
  Українська при цьому нормальна, читана: «Труба для теплої підлоги з кисневим бар'єром…».
- `specs` у i18n лежать **об'єктом**, не рядком JSON — та сама пастка, що ламала
  румунські характеристики в усіх 343 картках (виправлено 08.09).
- Характеристики беруть лише те, що публікує партнер (матеріал, розмір, бухта, бар'єр,
  країна). Тиску й температури в джерелі немає — вигадувати не стали.

### 🚫 Три винятки, щоб картка лишалась вітринною

- **Google Merchant:** `FEED_EXCLUDE` у `backend/routes/merchant.js` — артикул не йде
  в жоден із 6 мовних фідів.
- **Прайс для партнерів:** той самий список у `pipelines/price/generate.py` відсіює товар
  одразу після завантаження каталогу, тож його немає ні в XLSX, ні в CSV, ні у фіді.
- **«Власне виробництво»:** бейдж і однойменний акордеон на картці більше не показуються,
  якщо в базових характеристиках є «Бренд» і він не Termojet (`src/utils/brand.js`).
  На решті 343 карток нічого не змінилось — ключа «Бренд» у них немає.
- Дрібниця туди ж: у `docsMapping.js` для `SPY-16X2` стоїть exclusive-правило з порожнім
  списком — інакше на трубі висіли б категорійні інструкції на колектори.

## Сесія 2026-09-11 — Акція −25% у розділі «Акція» (лише на сайті)

> Знижка видима на сайті, у прайсі для партнерів ціна лишається роздрібною.
> Рішення власника від 11.09.2026: усі 18 позицій категорії «Акція», безстроково.

### 🔎 Що виявилось перед роботою

- **Знижки в базі не було ніколи.** `sale_price` заведено ще в червні, але перевірка живої
  бази й шести бекапів (4 черв → 8 вер) показала **0 у всіх 343 товарах**. Ціни самих 18
  позицій теж не мінялися жодного разу (`42020170 = 78 EUR` і в червні, і сьогодні), а
  вересневе підняття їх не чіпало — воно було по 157 **гривневих** позиціях, ці ж усі в EUR.
  Тобто −25% лишилися на старому WordPress і на новий сайт не переїхали; робимо з нуля.

### 💸 Знижка: `backend/scripts/apply-sale-2026-09.js`

- Пише `sale_price = price × 0.75`, **`price` не чіпає**. Прайс (`pipelines/price/generate.py`)
  бере з `/api/products` саме `price` — XLSX і CSV лишаються роздрібними без жодних правок.
- Відсоток, а не фіксована гривнева сума (як у `temp-price.cjs`): усі 18 в EUR, знижка
  лишається рівно 25% за будь-якого курсу НБУ, тож переводити товар у UAH не потрібно.
- Ідемпотентний, за замовчуванням лише показує; `--apply` пише, `--revert --apply` знімає.
  Запобіжник: якщо ціна в базі не та, з якої робився розрахунок (підняття цін або правка
  в адмінці), позиція пропускається з попередженням, а не перераховується наосліп.

### 🛒 Фіди Merchant: `g:price` роздрібна + `g:sale_price` акційна

- `backend/routes/merchant.js` (усі 6 мовних фідів) і `write_xml()` у `pipelines/price/generate.py`
  тепер віддають акційну ціну окремим полем. Якби у фіді лишилась сама роздрібна, Merchant
  побачив би на посадковій сторінці меншу суму, порахував це розбіжністю цін і зняв позиції
  з показу; задати ж ціну самою лише `g:sale_price` не можна.
- Статичний `termojet-feed.xml` перезбирати **тільки після** того, як знижка стоїть у базі сайту.

### 🏷 «−25%» числом замість слова «Акція»

- Бейдж на картці каталогу і тег біля ціни на сторінці товару показують відсоток
  (`salePercent()` у `src/utils/sale.js` рахує його з цін, окремо ніде не зберігається).
  Старий переклад лишився як `title` — на наведення й для читача екрана.

## Сесія 2026-09-08 — Гривневі ціни, правки характеристик, румунські specs

> Прод: бекап `backup-db.cjs pered-tsinamy-2026-09` → `docker compose cp` обох скриптів
> у `/app/backend/scripts/` → прогін без ключа, потім `--apply`. Перебудова образу не
> потрібна — БД у томі. Обидва скрипти ідемпотентні й розуміють `TERMOJET_DB`, тож
> репетиція йшла на знімку прод-бази, а не на робочій.

### 💰 Ціни: 153 картки (`apply-prices-2026-09.js`)

- **Рішення власника від 27.08.2026.** Насосні групи ×1.05, колектори в ізоляції ×1.07,
  без ізоляції ×1.08, серії 150(300) і Mega ×1.15, гідрострілки ×1.08 та інші правила —
  розрахунок і обґрунтування в `pipelines/price/pidnyattya-2026-08/`.
  Сума гривневої частини **2 453 610 → 2 700 520 грн (+10.06%)**, жодна ціна не впала.
- **Таблиця з 157 позицій вкладена в скрипт**: він виконується в контейнері, де репозиторію
  з розрахунком немає, а ціни мають бути видні в git-історії сайту.
- **Три кріплення КРН записані на сайті КИРИЛИЦЕЮ** (`КРН200V`), а в прайсі латиницею.
  Без цієї відповідності вони лишились би зі старою ціною при новій у прайсі.
- **Два запобіжники.** Товари під активною кампанією `temp-price.cjs` (два насоси APM-F
  до 17.09) пропускаються за станом із `data/temp-price-*.json`. Якщо поточна ціна не
  дорівнює тій, з якої робився розрахунок, позиція теж пропускається з попередженням —
  щоб не затерти ручну правку в адмінці.
- У прайсі 223 гривневі позиції, на сайті — 157: решта 66 (неізольовані «L», дві
  комплектації Mega, розмотувач) карток не мають узагалі й живуть лише в прайсі.

### 🔧 Характеристики: 27 карток шістьма мовами (`apply-site-fixes-2026-09.js`)

- **BOX2/BOX3** — «швидкорозʼєм» → «накидна гайка», габарити 500×400/600 → 492×466/676.
- **НГ-38А** — була 5 полів проти 10 у НГ-38. Тепер збирається як «характеристики НГ-38
  плюс поле привода», однаково в усіх шести мовах.
- **Насосні групи** — польський ключ `Głębokość` у не-польських мовах (4 картки × 4 мови),
  дописані й виправлені габарити пʼятьох EPP-груп.
- **Сміття з адмінки** — рядок-заголовок таблиці в характеристиках (9 карток × 6 мов) і
  слово «Редагувати» зі злиплим дампом у коротких описах (6 карток).
- **Двом змішувальним кранам написано короткі описи** шістьма мовами: у них увесь
  `short_desc` був дампом таблиці ВІД ПЕРШОГО символу, тож обрізка лишила б порожньо.

### 🇷🇴 Румунські specs: рядок JSON → обʼєкт, усі 343 картки

**Симптом.** `withI18n` віддає поля як є, а `ProductDetailPage` робить
`Object.entries(displaySpecs)`. Румунські `specs` лежали РЯДКОМ, тож таблиця
характеристик на румунських сторінках розсипалась на рядок НА КОЖЕН СИМВОЛ.
Решта мов зберігала обʼєкт і рендерилась нормально.

**Причина.** `products-ro-i18n.json` тримає specs рядком, а `patch-i18n-ro.js` клав
значення в базу дослівно. Виправлено в обох місцях: дані — блоком E скрипта правок,
причину — розбором у самому патчері, щоб повторний прогін не повернув рядки.

**Побічний наслідок, який довго ховався:** через різнотип `eachSpecs` мовчки пропускав
румунську, тож ro не правилась у ЖОДНОМУ блоці. Знайдено лише прогоном на знімку
прод-бази — на локальній копії це не спливло б.

### 🐞 Прайс: два дефекти зіставлення (`pipelines/price/generate.py`)

- **Дублі артикулів.** Коли два рядки старого прайсу лягають на один товар сайту,
  генератор вирішує «дубль чи інше виконання» порівнянням цін — але порівнював ціну
  САЙТУ з ціною зі старого XLSX. Поки вони збігалися, це працювало; після підняття
  чотири товари роздвоїлись. Тепер звіряються прайсові ціни обох рядків між собою.
- **Нечітке зіставлення ігнорувало числа в назві.** «Підставка … до 150кг» і «… до 260кг»
  дають подібність 0.94 і обидві сідали на єдину картку сайту (на 260 кг): рядок про
  150 кг місяцями тихо зникав з прайсу, а той, що лишався, носив чужу назву й ціну.
  Додано умову: числа в назвах мусять збігатися.

### 💶 Автоматика: дві занижені ціни на сайті

`903289 LT` і `903289 PR` стояли на сайті за 576 і 728.40 EUR, тоді як правильні —
634 і 800. Помилку **ховало правило `HIGHER_PRICE_SHEETS` у `generate.py`**: для розділу
«Автоматика» прайс бере вищу з двох цін, тож у прайсі роками стояло правильне число,
а на сайті — занижене, і звірка списувала різницю на це правило. Підтверджено власником
08.09.2026, виправлено тим самим скриптом цін (окремий блок таблиці з валютою EUR).

> Правило тепер бездіяльне — обидва джерела збігаються. Але поки воно є, наступна така
> розбіжність буде замаскована так само. Варте перегляду.

### 📥 Прайс на сторінці документів

За посиланням «Прайс-лист TERMOJET» роками віддавався **вихідний файл на 37 МБ від
06.04.2026** з дзеркала старого WordPress — тобто джерело конвеєра, а не зібраний прайс.
Замінено на наш файл від 08.09.2026: 4.4 МБ, `/uploads/files/price-termojet-2026-09-ua.xlsx`.

> Назва датована навмисно. `express.static` віддає `/uploads` з `maxAge: '7d'`, тож під
> сталою адресою відвідувач, який уже качав файл, тиждень отримував би стару ціну.
> Нова редакція прайсу — новий файл і правка URL у `src/data/files.js`.

Це єдина зміна сесії, що вимагала перезбірки образу (посилання зашите у фронт).
Перезапуск заразом підтвердив, що правки цін і румунських specs переживають
`docker compose up --build` — міграції старту їх не чіпають.

### 📄 Наслідки

- Мовні фіди Merchant генеруються з БД, тож нові ціни підхопились самі — вікна
  розбіжності між фідом і сторінкою не було.
- Прайс перезібрано повністю: `TERMOJET-прайс-2026-09.xlsx` (417 позицій), CSV і
  `termojet-feed.xml`. Звірка з сайтом: **339 позицій, нуль розбіжностей**.
- `seed-products.json` (151) і `src/data/products.js` (139) оновлені слідом. У seed
  пропущено `84030081`: там ціна 0, а не очікувана стара — розбіжність давня, окремо.
- **Підставки на 150 кг не існує** (підтвердив власник) — прибрано з прайсу новим
  фільтром `DROP_NAME` у `parse_price()`, аналогом `DROP_SKU`, але за назвою. Хибний
  рядок у старому XLSX роками маскувався самим дефектом зіставлення.

## Сесія 2026-09-03/04 — Тимчасові ціни в гривні, мовні посилання у фідах, затирання валюти щостарту

> Запит власника: поставити на дві позиції фіксовані ціни в гривні на два тижні, потім автоматично повернути попередні. Перевірка фідів після цього виявила два давніх баги — один із них дорогий.

Коміти: `763e17b` (кампанія цін) · `79a761c` (мовні `g:link` у фідах) · `269d093` (`migrateCurrency` + сторож кампанії).

### Тимчасові гривневі ціни на два насоси APM-F, до 17.09.2026 (`763e17b`)
- **Перевірка валюти (як просили):** обидва товари в БД у **EUR** — `APM-F 65/12-340` = 1340 €, `APM-F 40/12-250` = 1180 €, `sale_price` = 0 в обох. На сайті ціни рендеряться в гривні за курсом НБУ **+2.2 %** (`src/utils/currency.js`).
- **Чому не просто «перерахувати по курсу»:** гривнева цифра на сторінці — похідна від курсу НБУ, який тягнеться щодня. Якщо лишити `currency='EUR'` і поділити 73 000 на поточний курс, то вже наступного дня на сторінці буде не 73 000. Тож на час акції товар переводиться на `currency='UAH'` з фіксованою сумою — рівно та цифра, що просили.
- **Ціни за курсом на 04.09.2026** (НБУ 51.9382 × 1.022 = 53.0808): `65/12-340` показував 71 128 грн → став **73 000** (+2.6 %); `40/12-250` показував 62 635 грн → став **63 830** (+1.9 %). Це підвищення, не знижка — тому `price`, а не `sale_price` (інакше сайт намалював би «акцію» з закресленою нижчою ціною).
- **Новий скрипт `backend/scripts/temp-price.cjs`** — `--status` / `--apply` / `--auto` / `--revert`. Знімок «що з чого змінено» лягає в `data/temp-price-<id>.json` (змонтований том → переживає `up --build`).
- **Автоповернення 17.09.2026** — щоденний cron `root` о 06:00 запускає `--auto`; той мовчить, поки строк не вийшов. Щоденний, а не одноразовий: пропущений запуск (сервер лежав, контейнер перезбирався) наздоганяється наступного дня. Після повернення товари знову рахуються за курсом дня від 1340 і 1180 €.
- **Захист від затирання ручних правок:** якщо на момент повернення ціна в БД не та, яку виставив `--apply`, скрипт лишає товар як є й пише про це в лог замість того, щоб мовчки відкотити свіжу правку менеджера. Перекрити — `--force`.
- **Перевірено на копії прод-БД** (`VACUUM INTO`): apply → status → auto до строку (мовчить) → auto після строку з ручною правкою одного товару (другий повернувся, перший пропущений, стан збережено) → `--revert --force` (обидва повернулись у 1340/1180 EUR) → повторний `--auto` без стану (мовчить).
- **Перевірено на проді:** `/api/products`, відрендерена сторінка в uk/de/pl, JSON-LD у сирому HTML, усі 6 Merchant-фідів, кошик. Сусідні насоси (`65/15`, `40/15`) лишились у EUR — зачепило рівно дві позиції.

### Google Merchant: мовні фіди вели на україномовні URL (`79a761c`)
- Знайдено під час перевірки цін у фідах. `g:link` у **всіх 6 фідах** формувався як `BASE + /catalog/<cat>/<slug>` без мовного префікса (`backend/routes/merchant.js`). Тобто в німецькому фіді назва й опис німецькі, а посилання — на українську сторінку. Merchant Center звіряє мову фіда з мовою посадкової сторінки й за розбіжність відхиляє позиції.
- Додано `LANG_PREFIX` (дзеркало однойменної мапи в `backend/server.js`); `g:link` і `<link>` каналу тепер ведуть на `/en|/pl|/de|/fr|/ro`. Українському фіду префікс порожній — його URL не змінились.
- Перед правкою перевірено, що цілі існують: усі 6 мовних URL товару віддають **200**, **самоканонічні**, `robots: index, follow`, hreflang×7. Тобто фід веде на індексовану сторінку потрібною мовою, а не на редирект.
- Після деплою: **2052 посилання** в 6 фідах, 0 без правильного префікса; вибірка **30 посилань** з усіх мов — усі 200, без редиректу, самоканонічні, індексовані.
- Баг давній, до зміни цін стосунку не має.

### 🔴 migrateCurrency затирала валюту щостарту — ціна на вітрині стала 3.87 млн грн (`269d093`)
- **Знайдено через 4 хв після деплою фікса фідів** — статус кампанії показав «73 000 **EUR**» замість «73 000 грн».
- **Причина — той самий клас бага, що виправлений 03.09 у `migrateSeo()`, у сусідній функції:** `migrateCurrency()` робила БЕЗУМОВНИЙ `UPDATE products SET currency = ?` зі `seed-products.json` щостарту (коментар прямо казав «seed — джерело істини, самовиправляється щостарту»). `migrateSeo` тоді полагодили, а цю пропустили.
- **Чому вийшло так дорого:** ціну не чіпає жодна міграція, а валюту чіпала. Пара розʼїхалась — `price=73000` з `currency='EUR'` → сторінка й JSON-LD показали **3 874 901 грн**. Тобто баг не просто відкочував зміну, а робив гірше, ніж було до неї. Так само мовчки гинула б будь-яка зміна валюти через адмінку.
- **Виправлено:** backfill лише порожніх (`COALESCE(TRIM(currency),'')=''`), з логом. Валюту на проді повернуто одразу, ще до правки коду — сторінка була хибною ~4 хв.
- **Аудит решти міграцій:** `seedProducts`/`seedBlog`/`seedPortfolio` сіють лише в порожню таблицю; `migrateI18n`/`migratePortfolioCols` лише додають колонки; `migrateSpecsArticle` синхронізує `specs.Артикул` з `sku` **у самій БД**, не зі seed. Тобто `migrateCurrency` була останньою міграцією, що безумовно перезаписувала живі дані зі seed-файлу.
- **Правило на майбутнє (записане і в DEPLOY.md):** будь-яка міграція, що читає `seed-*.json`, має ЗАПОВНЮВАТИ порожнє, а не перезаписувати живе — інакше вона тихо їсть правки з адмінки.
- **Друга лінія захисту в `temp-price.cjs`:** щоденний `--auto` тепер ще й звіряє цілісність кампанії. Кампанійна ціна при чужій валюті — підпис машинної псувальниці (ніхто не міняє в адмінці саму лише валюту, зберігши акційну суму), тож валюта повертається з гучним записом у лог. Якщо ж змінилась і ціна — це схоже на свідому правку менеджера, і сторож мовчить. Обидві гілки перевірені на копії прод-БД.
- **Вирішальна перевірка:** ребілд `up -d --build` і окремий `restart` — ціни й валюта вижили обидва рази. Розподіл по 343 товарах після фікса: 184 EUR / 159 UAH, без порожніх.

### Стан на кінець сесії
| | |
|---|---|
| Сторінки товарів | 73 000 ₴ / 63 830 ₴ |
| JSON-LD | `"73000"/"UAH"`, `"63830"/"UAH"` |
| Merchant-фіди ×6 | ціни правильні, посилання мовні |
| Автоповернення | 17.09.2026, cron стоїть, прогнаний у порожньому оточенні |
| Локально / origin / прод | `269d093` |

**Що варто зробити власнику:** після 17.09 звіритись, що ціни повернулись (`docker compose exec app node backend/scripts/temp-price.cjs` покаже «не застосована»), і перевірити в адмінці товари, чиї SEO-поля чи валюту редагували до 03.09 — ті правки могли загинути під час рестартів, поки обидва баги були живі.
...
| Сторінка | Було | Стало |
|---|---|---|
| Головна, моб | 17 008 мс | **1 152 мс** 🟢 |
| Головна, деск | 16 840 мс | **4 388 мс** |
| Товар, моб | 17 668 мс | **2 556 мс** 🟡 |
| Товар, деск | 17 836 мс | **4 420 мс** |
| Категорія, моб | 15 180 мс | **5 612 мс** |
| Категорія, деск | 21 312 мс | **6 580 мс** |
| /about, моб | 33 968 мс | **7 476 мс** |
| /about, деск | 53 412 мс | **12 600 мс** |

**CLS — зелений на всіх типах сторінок і мовах.** Найбільший зсув сайту (товар, 0.6166) виправлено окремо (див. нижче) → 0.0173.
...
6. **model-viewer поза критичним шляхом** (коміт `2796890`). 1017 кБ тягнулись одразу на кожному товарі з 3D-моделлю, навіть якщо 3D не відкривали. Тепер: відкрито 3D — вантажимо негайно, інакше — `requestIdleCallback`. Перевірено: завершується на 14.2 с замість критичного шляху.

7. **Описи товарів прибрано зі списку** (коміт `c8a27aa`). Найважче поле: 1.82 МБ на мову. Перевірила на даних, що просто перейти на `shortDesc` не можна — фільтр «Тепла підлога» дав би 5 збігів замість 48; обрізати опис теж (при 1200 симв — 233 збіги з 257). Тому: ознаки «Призначення» рахує сервер тими самими регекспами (паритет 993/993), 62 картки без `shortDesc` отримали витяг з опису, а `ProductDetailPage` тягне повний товар сам із `/api/products/:slug`. **Список: 10.5 МБ → 0.60 МБ.**
8. **Другорядні довідники — на простій браузера** (коміт `e621c5b`). AppContext тягнув 8 запитів одразу на кожній сторінці; блог (517 кБ) вантажився навіть на сторінці товару. Негайним лишився тільки `settings` (телефон у шапці).
9. **Фото товарів → WebP** (коміт `e621c5b`). 113 файлів понад 100 кБ: 41.8 МБ → 9.0 МБ. Middleware віддає `<file>.png.webp` за заголовком `Accept` — той самий прийом, що вже був для `.gz` 3D-моделей. **URL не змінюються, тому посилання в БД (404 штуки) чіпати не треба.**
10. **GTM стартує за взаємодією або простоєм, стель 2.5 с** (коміт `33925eb`). Через GTM іде 1.06 МБ: gtm.js 380 + gtag 566 + Clarity 73 + Binotel. `dataLayer` створюється одразу, тож події застосунку не губляться. ⚠️ Стель саме 2.5 с через Binotel — підміна номерів має встигнути до реального дзвінка. ⚠️ Компроміс: хто піде швидше за 2.5 с і не торкнеться сторінки — у статистику не потрапить.
11. **CLS товару 0.6166 → 0.0173** (коміт `7628c41`). Причина виявилась не в пізніх картинках: сторінка мала 2645px (SSR-блок) → **900px** (короткий лоадер, рівно висота вьюпорта) → 3596px. Футер (`mt-auto`) заходив у вьюпорт і виходив назад. Заглушці задано `min-h-[150vh]` — футер лишається під згином на всіх трьох станах.
12. **/about: LCP 53 с** (коміт `692e03b`). LCP-елемент — `about-hero.png` 1.3 МБ, який чекав у черзі за YouTube-плеєром (iframe вантажився одразу, 530 кБ CSS + 154 кБ документа). WebP-middleware розширено на кореневі файли `public/` (20 файлів: 19.7 МБ → 1.11 МБ), iframe отримав `loading="lazy"`.
...
### 📋 Лишилось
- **LCP 4.4–6.6 с на десктопі** — далі впирається в кількість запитів (63–85) і залишкові зображення. Наступні кроки: `preload` для LCP-зображення категорії, responsive-розміри (`srcset`) замість одного банера 2320px на всі екрани.
- **`/about` 12.6 с** — там ще `about-factory.mp4` на 12 МБ.
- **Польові дані Google** оновлюються 28 днів — реальний ефект дивитись у GSC → Core Web Vitals приблизно за місяць. Усі цифри тут лабораторні.
...
## Сесія 2026-08-26 — Робочий тиск насосних груп: 6 бар → 3 бар

> Прод: `docker cp backend/scripts/apply-ng-tysk-3bar.js <container>:/app/backend/scripts/` + `docker compose exec -T app node backend/scripts/apply-ng-tysk-3bar.js`. Перед запуском знято бекап `termojet.db.bak-2026-08-26`. Перебудова образу не потрібна — БД у томі.

### 🔧 Дані: 24 картки насосних груп

- **Підстава.** Інструкція Termojet 2020 року (на сайті `id 12` і `id 13`) у таблиці характеристик дає 6 бар, а в п. 2.7 — «випробування тиском не більше 3 бар». Польські GPSR-наліпки друкували 3 bar, українські — 6. Рішення замовника 26.08.2026: вірні **3 бар**, усі документи вирівнюються по ньому.
- **Скрипт `backend/scripts/apply-ng-tysk-3bar.js`** — ідемпотентний, вибірка за назвою товару (`Насосна група` + `НГ-46/47/48/51/52/67`), а не пошуком по всій базі. Правляться **всі поля картки**, не лише `specs`: тиск згадується ще в `description`, `short_desc`, `meta_description` і в перекладах `i18n` (en, pl, fr, de) — інакше на сторінці товару поряд стояли б 3 бар у характеристиках і 6 бар у тексті.
- **Чужого не зачепило.** «6 бар» лишилось у повітровідвідників `41015110/41020110/41025110`, труби PE-RT `520472` і насосів TBE — це їхній власний тиск. Контроль вбудований у скрипт.
- **Джерела теж виправлено:** `backend/seed-products.json` (53 заміни), `src/data/products.js` (50), `../app/public/data/devices_v5.json` (78 замін у шести мовах — конструктор).
- Перевірено на живому API: 24 з 24 груп віддають «до 3 бар», жодної з «6 бар».

### 📄 Наслідки за межами сайту

- Обидві партії наліпок (`pipelines/nalipky/ua` і `/epp`) перезібрані; обхід `zaminy` у генераторі наліпок знято — тиск тепер береться просто з картки.
- Гарантія на наліпках виправлена з 12 на **24 місяці** — інструкції партії й гарантійний талон казали 24 від початку.
- Прайс (`pipelines/price`) бере номенклатуру з `/api/products`, тож підтягне нове значення при наступній перезбірці.
