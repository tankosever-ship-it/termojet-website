...
- [x] **Видимі описи категорій** (9 лип) — `categories.js` desc.uk/en збагачено до rich-тексту (pl/fr/de збережено), `CatalogPage.jsx`: прибрано бляклий підзаголовок у шапці, додано читабельний блок-абзац під шапкою. Дає UX + **реальний on-page body-текст** (лікує «thin content»). Синхронізовано з meta (`CATEGORY_META` у server.js — тримати в парі, бекенд CJS не читає src).
- [x] **1.2 Відгуки** (10 лип) — клієнти додали **5 реальних відгуків** (усі 5★) через адмінку; колега зробив сторінку `/reviews`. Підняли **E-E-A-T 61→68**. ⚠️ Це відгуки про компанію/сервіс (не про товари) → Google **НЕ показує зірки** (self-serving policy). Для ⭐ біля товарів потрібні per-product відгуки.

### Ре-аудит 10 лип 2026 — score 81/B (0 помилок)
`squirrel audit -C full --refresh`. **80→81, grade B, `failed` 5→0.** Core SEO 94→**99**, Content 82→**93**, Structured Data/Local SEO **100**, E-E-A-T 61→**68**, Performance **94**. Прогрес від старту: **73→81**.
- 🩹 Виправлено **5 невалідних Product-schema** (`2bf6125`): 3 товари без ціни (`price=0`) віддавали Product без offers → тепер лише Breadcrumb+Organization.
- 🩹 Виправлено **стрей `/>` угорі сайту** (`8de5f09`): canonical-regex не з'їдав кінцевий ` />`.
- ⚠️ **ГОЧА:** ре-імпорт товарів колеги стер дедуп UA-`seo_title` (дублі повернулись) → відновлено повторним прогоном `seo-fix-titles.cjs`. Ганяти після кожного ре-імпорту (див. `DEPLOY.md`).
- Залишок — відомі артефакти сирий-vs-рендер (Legal 44, thin-content, no-main, orphan) + CSP unsafe-inline (свідомо) + numeric slugs (info). Нічого нового/критичного.

| 2026-07-10 | ре-аудит | 81/B, failed 0. Фікси: Product-без-offers, стрей `/>`. Відгуки 5★ (E-E-A-T→68). Гоча: ре-імпорт стирає дедуп title. | 80→81 |
