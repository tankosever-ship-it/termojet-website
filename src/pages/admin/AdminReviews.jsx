...
export default function AdminReviews() {
  const { reviews, setReviews, isAdminAuth } = useApp()
  ...
  function save() {
    ...
    setReviews(prev => [{ ...form, id: Date.now().toString(), createdAt: new Date().toISOString() }, ...prev])
      ...
      <div className="bg-[var(--primary)] text-white px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-white/60 text-xs">{reviews.length} відгуків</div>
          </div>
        </div>
          ...
          <div className="space-y-3">
            {reviews.map(r => (
              <div key={r.id} className="card p-4 flex gap-4">
                ...
                <button onClick={() => { if (confirm('Видалити?')) setReviews(prev => prev.filter(x => x.id !== r.id)) }}
                  className="p-1.5 text-gray-400 hover:text-red-500 flex-shrink-0 self-start">
                  <Trash2 size={14} />
                </button>
              </div>
            ))}
